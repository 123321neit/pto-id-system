...
Основание: `docs/12-database-schema-v1.md`, `docs/13-domain-lifecycle-immutability-validation-v1.md`, `docs/14-backend-api-architecture-v1.md`, ADR 0001-0008, анализ АОСР и реестра.
...
Section-template amendment note, 2026-06-23: working document contracts must
expose strict `linked`/`manual` template mode under ADR 0007. Linked mode stores
references and resolves current `SectionTemplate`/library data; an explicit
...
valid command surface. ADR 0008 supersedes object-level template scope: future
implementation uses `Object -> DocumentationSection -> ID folders -> documents`,
and the UI-facing template is `настройки шаблона раздела`.

Backend contract amendment note, 2026-06-23: the future command/read-model
contract for `SectionTemplate`, user-defined sections, user-defined ID folders,
section/folder-scoped document creation, section/folder numbering proposals and
linked/manual AOSR boundaries is now explicitly recorded below. This is still a
conceptual application contract: no route list, OpenAPI, DTO serialization,
Prisma schema, SQL/migrations, repository, persistence adapter, queue, renderer
or production backend code is authorized by this amendment.
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_document`                         | Select approved `document_type`, one `object_id`, one `section_id`, required valid folder placement inside that section, initial act-owned structured values, numbering intent/policy and `SectionTemplate`/form-template references. New acts start `linked` unless an explicit later policy says otherwise. | New `document_id`, initial `DRAFT` linked working state/version, section/folder placement, assigned/proposed number outcome and initial draft findings.                                                                              | Idempotency required when creation may be retried; validates scope/type/section/folder; never creates released revision or automatic template snapshot.   |
...
| `switch_document_template_mode_to_manual` | Explicitly resolve the complete current `SectionTemplate`/library state and switch the whole template-owned section to manual.                                                                                                                                                                                | Manual working version plus one complete `manualTemplateSnapshot`.                                                                                                                                                                   | Requires expected working/template versions, confirmation and idempotency; never creates scattered per-field overrides.                                   |
| `return_document_to_section_template`     | Explicitly discard the working manual template state and resume linked resolution from the current section template.                                                                                                                                                                                          | Linked working version and current resolved template context; released history remains unchanged.                                                                                                                                    | Requires expected version and confirmation; must not mutate any prior released revision.                                                                  |
...
### 7.1 SectionTemplate command contracts

These command contracts refine the `ObjectDocumentationContext` / typed document
boundary. `DocumentationSection` is a user-defined section inside an object.
`SectionTemplate` owns section-specific live assignments and repeated print
values; `Document` owns act-specific payload, links, lifecycle and released
revisions.

| Command/read                                        | Intent and payload/read semantics                                                                                                                                                    | Result/read semantics                                                                                                                                                                                 | Rules                                                                                                                                                                                                                                                                       |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_documentation_section`                      | Create a user-defined section such as `Вентиляция`, `Отопление`, `ВК`, `Система В1` or another user name inside one object.                                                          | New section id, initial section-template settings and visible section list update.                                                                                                                    | Requires object access; section names are not a fixed enum; does not create folders or documents by itself.                                                                                                                                                                 |
| `read_document_creation_context`                    | Query one object, one documentation section and one user-defined ID folder before creating a document. Include selected document type if already chosen.                             | Approved document types, section identity/title, folder identity/title, current section-template summary, numbering policy, proposed next number, ID package scope and visible capability/actions.    | Query only; it does not create a draft, reserve a number or mutate sequence state.                                                                                                                                                                                          |
| `update_section_template_texts`                     | Change section-level repeated print values: section title/subscript if applicable, project documentation, point 6 compliance text, work contractor, additional info and copies line. | New section-template version, updated resolved template read and affected current linked-read/stale hints.                                                                                            | Requires expected section-template version; cannot edit a released revision, manual snapshot or act-owned fields.                                                                                                                                                           |
| `configure_section_numbering_policy`                | Change numbering scope, prefix, suffix and sequence behavior for a section/document type, including section-wide or per-folder scope.                                                | New numbering/template configuration version and updated creation-context proposals.                                                                                                                  | Requires expected configuration version; affects future proposals/new documents only. Existing documents change only through `renumber_documents` or document revision commands.                                                                                            |
| `add_section_template_counterparty_assignment`      | Link/select or create-through-library a global organization for section print header with role/title/order/subscript/details.                                                        | New assignment id/version and resolved linked context for active reads.                                                                                                                               | Requires expected section-template version and same workspace; no act-only organization source is created.                                                                                                                                                                  |
| `update_section_template_counterparty_assignment`   | Edit section-specific role/title/order/subscript/details for an existing counterparty assignment.                                                                                    | New assignment/template version and updated linked read.                                                                                                                                              | Does not mutate historical released output; global library fields are changed only through the library owner's command.                                                                                                                                                     |
| `reorder_section_template_counterparties`           | Reorder print header organization blocks for future/current linked working acts in one section.                                                                                      | New template version and resolved ordering.                                                                                                                                                           | Requires expected version; released revision/package order remains the order captured at release/build.                                                                                                                                                                     |
| `remove_section_template_counterparty_assignment`   | Remove a section-template organization assignment from active linked context.                                                                                                        | New template version plus draft/readiness findings where active linked acts lose required context.                                                                                                    | Must expose impact; cannot remove frozen historical output.                                                                                                                                                                                                                 |
| `add_section_template_representative_assignment`    | Link/select or create-through-library a global representative into a section signature group/role with section-specific position, authority, NRS and subscript.                      | New representative assignment/group version and updated linked signatory context.                                                                                                                     | Requires expected version; no direct free-text final act source in linked mode.                                                                                                                                                                                             |
| `update_section_template_representative_assignment` | Edit section-specific representative context or group title/order.                                                                                                                   | New template version and resolved group/member context.                                                                                                                                               | Global representative profile fields are changed through library owner command; manual/released outputs do not float.                                                                                                                                                       |
| `move_section_template_representative_assignment`   | Move a representative between groups or reorder within a group.                                                                                                                      | New template version and resolved linked signature order.                                                                                                                                             | Requires expected version; active linked reads may change, frozen outputs remain unchanged.                                                                                                                                                                                 |
| `remove_section_template_representative_assignment` | Remove a section representative assignment from active linked context.                                                                                                               | New template version plus draft/readiness findings where active linked acts lose required signatory context.                                                                                          | Historical released/manual snapshots remain intact.                                                                                                                                                                                                                         |
| `copy_section_template_to_section`                  | Copy `настройки шаблона раздела` from one section to another section in the same object or another object.                                                                           | Target section gets its own new section-template version with copied repeated texts, numbering policy, organization/representative library links and section-specific labels/order/groups/subscripts. | Requires source read and target mutate permission; does not copy folders, documents, drafts, released revisions, manual snapshots, packages, generated artifacts or global library records themselves. Cross-workspace copy requires a later explicit import/export policy. |
...
| `return_document_to_section_template`               | Explicit user decision to discard working manual template values and resume linked resolution.                                                                                       | New linked working version and current resolved section-template context.                                                                                                                             | Requires expected working version; no released revision or package snapshot is rewritten.                                                                                                                                                                                   |

Section-template read guarantees:

- `SectionTemplate` editor reads include current version, live-chain
...
- `create_document` from a folder uses the same creation context and returns
  the selected section/folder placement; section and folder names are user text,
  not fixed enums.
- Intermediate ID is folder-scoped. Final ID is section-scoped.

---

## 8. Numbering and Folder Command Contracts

Folders organize business work inside one section; they do not own document content or turn into a general file drive. Numbering values remain structured: scope/policy, prefix, sequence, suffix and rendered value.

| Command                                         | Intent and payload semantics                                                                                                       | Result semantics                                                                                                          | Rules / effects                                                                                                                        |
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `create_folder`                                 | Create a business folder under an optional same-tree parent with title/order in one section.                                       | New folder identity and tree version.                                                                                     | Requires expected tree version where tree already exists; prevents cross-section parent/cycle.                                         |
...
| Object dashboard                     | Object id/name/status/context, section list, section/folder/document/package counts, validation `ERROR`/`WARNING` counts, latest section package freshness/build state, pending AI/source work and recent safe activity summary.                                                                                                                          |
| Section template settings view       | Section-template id/version, section identity/title, live-chain explanation, repeated print values, numbering policy/proposal example, organization assignments with library provenance, representative groups/members with section-specific display context, copy-template action/impact hints, validation/impact hints and permitted commands.          |
| Document creation context            | One object plus selected user-defined section and folder, available document types, current section-template summary, numbering policy, proposed next number, ID package scope, disabled/deferred document types and create permissions.                                                                                                                  |
| Folder tree/document list            | Section-scoped folder hierarchy/order/status/version, selected-folder placement list, document type/number/date/lifecycle/latest revision/working-change marker, scheme/package representation, validation/stale badges and permitted commands.                                                                                                           |
...
| Package builder view                 | Package identity/configuration version/title/scope, selected section/folder component order, exact referenced releases/evidence/schemes/registry config, readiness findings, current/latest snapshot state, async operation progress/failure/retry, release/download/stale indicators.                                                                    |
...
| Search results                       | Tenant-filtered result kind/id/title or canonical identifier, object/section/folder context, lifecycle/status/freshness, validation/stale cue where useful, index freshness caveat and link to authoritative read model.                                                                                                                                  |
...
- [x] Section-template and section/folder-scoped document creation contracts preserve `global libraries -> SectionTemplate -> linked act`, explicit whole-act manual snapshots and no partial template overrides.
