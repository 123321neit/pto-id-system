...
the certificate library, and applications remain derived from selected
certificates plus selected drawings, schemes and attachments. The preview
wording for point 4 was corrected closer to the real AOSR wording:
`4. Предъявлены документы, подтверждающие соответствие работ предъявляемым к ним
требованиям:`. TODO for future separate steps: move default compliance text to
object settings with per-act override, and expose application inclusion
checkboxes. No backend, persistence, Prisma/schema/migrations, routes,
auth/session, uploads, OCR/AI, real DOCX/PDF generation, share codes or
production business logic were introduced.
