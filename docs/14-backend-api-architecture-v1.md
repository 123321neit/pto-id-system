...
Основание: `docs/12-database-schema-v1.md`, `docs/13-domain-lifecycle-immutability-validation-v1.md`, ADR 0001-0008, анализ АОСР и реестра.
...
resolve template-owned data through global libraries and `SectionTemplate`
(`настройки шаблона раздела`);
manual acts own one complete snapshot; released revisions/packages freeze exact
output separately. ADR 0008 supersedes object-level template scope: the
canonical hierarchy is `Object -> DocumentationSection -> ID folders ->
documents`, intermediate ID is folder-scoped, and final ID is section-scoped.
Older `ObjectTemplate`, `adopt_company_snapshot` and
`configure_representative_defaults` command names are noncanonical for future
implementation.

Backend contract amendment note, 2026-06-23:

The conceptual backend contract for `SectionTemplate`, user-defined
documentation sections, user-defined ID folders, section/folder-scoped document
creation and strict linked/manual AOSR behavior is now recorded in this document and in
...
| Aspect           | Definition                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Responsibility   | Рабочий контекст строительного объекта: object identity, user-defined documentation sections, section-template settings, project drawing sets and section/folder numbering/template settings.                                                                                                                                                                                                         |
| Owns             | Object identity/settings, `DocumentationSection`, `SectionTemplate` references plus section-specific labels/groups/order/repeated texts, `ProjectDrawingSet` как owned entity and applicable numbering configuration.                                                                                                                                                                                 |
...
| Main commands    | `create_object`, `update_object_context`, `create/update/reorder/archive_documentation_section`, `update_section_template_texts`, `configure_section_numbering_policy`, `copy_section_template_to_section`, `add/update/reorder/remove_section_template_counterparty`, `add/update/move/remove_section_template_representative_assignment`, `create_or_update_project_drawing_set`, `archive_object`. |
| Main read models | Object dashboard header, section list, section template settings view, resolved library assignments with provenance, document creation context, drawing-set summary and readiness inputs.                                                                                                                                                                                                             |

### 3.4 FolderTree

| Aspect           | Definition                                                                                                                                                                                                                        |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Responsibility   | Section-scoped business grouping документов, схем и package views: этапы, участки, периоды and other user-defined ID folders inside one documentation section.                                                                    |
| Owns             | Folder nodes inside one `DocumentationSection`, hierarchy, placements, sibling order, business clone/move operation outcome.                                                                                                      |
| Does not own     | Typed document content/lifecycle, evidence files, package build contents, произвольную файловую библиотеку.                                                                                                                       |
| Main commands    | `create_folder`, `rename_folder`, `move_folder_node`, `move_folder_item` с `keep_numbering`/`recalculate_numbering`, `clone_folder` with strategy, `archive_folder`, `restore_folder`. All folder commands are scoped by section. |
...
| Main read models | Executive scheme picker/detail view, scheme usage/history, object/section/folder scheme list and missing-file/readiness state.               |
...
| ---------------------------------------------- | --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `create_document`                              | Создать новый АОСР либо другой утвержденный typed act в разделе/папке.      | Проверить workspace/object/section/folder/type, создать `DRAFT` identity and working state; type immutable.                                  |
...
### 5.1.1 SectionTemplate and section/folder-scoped creation contract

This subsection fixes the future backend contract for the frontend model now
accepted by ADR 0008: `Object -> user-defined DocumentationSection ->
user-defined ID folders -> documents`, with active linked AOSR resolving
printable repeated values through `global libraries -> SectionTemplate`.

| Command family                                                      | PTO intent                                                                                                                                                                            | Required backend effect                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `create/update/reorder/archive_documentation_section`               | Создать или настроить пользовательский раздел ИД внутри объекта: `Вентиляция`, `Отопление`, `ВК`, `Система В1` or any user-defined name.                                              | Mutate only section identity/order/status; sections are not a fixed enum; archiving preserves documents, released revisions and package history according to lifecycle rules.                                                                                                                                                                          |
| `read_document_creation_context`                                    | Открыть selector `Создать документ` inside one folder of one section.                                                                                                                 | Query only: return object, section, folder, approved document types, next-number proposal, section-template summary, ID package scope and permitted actions. It does not reserve a number or create a draft.                                                                                                                                           |
| `create_document` with section/folder placement                     | Создать документ выбранного type inside one user-defined folder of one section.                                                                                                       | Validate workspace/object/section/folder/type, assign initial placement, create linked working draft, attach current section-template/form references and allocate numbering according to policy if automatic numbering is requested.                                                                                                                  |
| `update_section_template_texts`                                     | Изменить repeated section-level print values: section title/subscript if applicable, project documentation for the section, compliance text, work contractor, copies/additional info. | Version-check `SectionTemplate`, update only template-owned current values, expose stale/current-read implications for active linked views without mutating manual/released snapshots.                                                                                                                                                                 |
| `configure_section_numbering_policy`                                | Настроить section/folder numbering: prefix, suffix, scope and sequence behavior.                                                                                                      | Version-check numbering/template configuration; affect future proposals/new documents only unless an explicit `renumber_documents` command is separately confirmed.                                                                                                                                                                                    |
| `add/update/reorder/remove_section_template_counterparty`           | Назначить global organization to section print header with section-specific role/order/subscript/details.                                                                             | Validate global library reference or create-through-library outcome, store section-template assignment/context, update active linked reads; never create act-only free text as normal source.                                                                                                                                                          |
| `add/update/move/remove_section_template_representative_assignment` | Назначить global representative into section-level signature groups with role/order/section-specific display values.                                                                  | Validate library reference, group membership and same workspace; update section-template assignment/context; linked acts resolve it live while manual/released outputs remain frozen.                                                                                                                                                                  |
| `copy_section_template_to_section`                                  | Скопировать настройки шаблона раздела into another section in the same object or another object.                                                                                      | Create a target section-template version that copies repeated texts, numbering policy, organization/representative assignment links and section-specific labels/order/groups/subscripts. It does not copy folders, documents, drafts, released revisions, manual snapshots, issued packages, generated artifacts or global library records themselves. |
...
| `return_document_to_section_template`                               | Вернуть working act from manual snapshot to live template resolution.                                                                                                                 | Discard working manual template values for that draft and resume linked resolution from current section template; released revision history remains unchanged.                                                                                                                                                                                         |

Backend rules for this contract:

- Section names and folder names are user-defined strings, not fixed engineering
  or month/period taxonomies.
- Folders belong to one section; final ID/package scope is the section, while
  intermediate ID scope is a folder inside that section.
...
- Changing section template, global organization/representative library records
...
| Search results                       | Быстро найти акт, сертификат или схему.                  | Tenant-filtered result kind/object/section/folder/status, authoritative destination, staleness/index recency indication if needed.                                                                  |
